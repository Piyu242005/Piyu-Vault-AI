...
from app.services.storage_service import absolute_path, delete as delete_storage, save_bytes
...
async def upload_file(file: UploadFile = File(...), db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    try:
        storage_name = save_bytes(user.id, original_name, content)
    except ValueError as exc:
        raise HTTPException(status_code=413, detail=str(exc)) from exc

    document = Document(user_id=user.id, name=original_name, file_type=file.content_type or "application/octet-stream", size=len(content), storage_path=storage_name)
    db.add(document)
    ...
    if Path(original_name).suffix.lower() in TEXT_EXTENSIONS:
        try:
            ...
            if text.strip():
                ingestion_service.ingest_text(text, {"user_id": user.id, "document_id": document.id, "source": original_name})
        except Exception:
            pass
    return document
...
async def download_file(file_id: str, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    result = await db.execute(select(Document).where(Document.id == file_id, Document.user_id == user.id))
    document = result.scalar_one_or_none()
    if not document:
        raise HTTPException(status_code=404, detail="File not found")
    path = absolute_path(user.id, document.storage_path)
    if not path.exists():
        raise HTTPException(status_code=404, detail="Stored file is missing")
    return FileResponse(path, media_type=document.file_type, filename=document.name)
